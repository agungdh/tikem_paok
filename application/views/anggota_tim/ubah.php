...
<div class="row">
  <div class="col-md-12">
    <div class="tile">
      ...
      <div class="tile-body">
        <form method="post" action="<?php echo base_url('anggota_tim/aksi_ubah'); ?>" enctype="multipart/form-data">
            ...
            <select class="form-control select2" required name="data[mahasiswa_id]">
              ...
              foreach ($this->db->query("SELECT *
                                        ...
                                        WHERE id NOT IN (SELECT mahasiswa_id
                                                         FROM detil_tim
                                                         WHERE tim_id = ?)
                                        OR id IN (?)", [$data['detil_kegiatan']->id, $data['anggota_tim']->mahasiswa_id])->result() as $item) {
                ?>
